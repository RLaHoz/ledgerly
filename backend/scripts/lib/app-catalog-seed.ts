...
export const CATEGORY_BLUEPRINTS_WITH_UPDATED_ICONS: CategoryBlueprint[] =
...
export async function seedAppCatalog(
  prisma: PrismaClient,
  options?: { resetExistingData?: boolean },
): Promise<{ categoryCount: number; subcategoryCount: number }> {
  const resetExistingData = options?.resetExistingData ?? false;
  ...
  const expectedSubcategoryKeys = new Set<string>();

  for (const [categoryIndex, category] of CATEGORY_BLUEPRINTS_WITH_UPDATED_ICONS.entries()) {
    expectedCategorySlugs.add(category.slug);
    ...
    for (const [subIndex, subName] of subcategoryNames.entries()) {
      const subcategorySlug = slugifyCatalogValue(subName);
      expectedSubcategoryKeys.add(`${category.slug}:${subcategorySlug}`);
  ...
  const existingCategories = await prisma.appCategoryList.findMany({
    select: { id: true, slug: true },
  });

  for (const category of existingCategories) {
    await prisma.appCategoryList.update({
      where: { id: category.id },
      data: { isActive: expectedCategorySlugs.has(category.slug) },
    });
  }

  const existingSubcategories = await prisma.appSubcategoryList.findMany({
    select: {
      id: true,
      slug: true,
      appCategory: { select: { slug: true } },
    },
  });

  for (const subcategory of existingSubcategories) {
    const key = `${subcategory.appCategory.slug}:${subcategory.slug}`;
    await prisma.appSubcategoryList.update({
      where: { id: subcategory.id },
      data: { isActive: expectedSubcategoryKeys.has(key) },
    });
  }
